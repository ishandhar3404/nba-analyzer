...
from services.stats_service import get_regular_season_stats, get_playoff_stats
...
def main():
    ...
    try:
        ...
        stats1 = get_regular_season_stats(player_id1, year1) if season_type1 == "regular" else get_playoff_stats(player_id1, year1)
        print(f"Fetching data for {name2} ({year2})...")
        stats2 = get_regular_season_stats(player_id2, year2) if season_type2 == "regular" else get_playoff_stats(player_id2, year2)
        
        # Format player names with season info
        player1_full = f"{name1} ({year1} {season_type1})"
        player2_full = f"{name2} ({year2} {season_type2})"
        ...
        print("Note: Playoff data might not be available for current season")
